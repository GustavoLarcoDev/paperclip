...
export function SidebarAccountMenu({
  ...
  const displayName = session?.user.name?.trim() || t("nav.accountMenu.boardFallback");
  const secondaryLabel =
    session?.user.email?.trim() || (deploymentMode === "authenticated" ? t("nav.accountMenu.signedIn") : t("nav.accountMenu.localBoard"));
        ...
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <button
              ...
              aria-label={t("nav.accountMenu.openMenu")}
            >
              <Avatar size="sm">
          ...
          >
            ...
            <div className="flex flex-1 flex-col gap-0.5 border-t border-border px-2.5 pb-2.5 pt-2">
              <MenuAction
                label={t("nav.accountMenu.settings")}
                icon={Settings}
                href="/company/settings"
                onClick={closeNavigationChrome}
              />
              <MenuAction
                label={t("nav.accountMenu.viewProfile")}
                icon={UserRound}
                href={profileHref}
                onClick={closeNavigationChrome}
              />
              <MenuAction
                label={t("nav.accountMenu.editProfile")}
                icon={UserRoundPen}
                href={PROFILE_SETTINGS_PATH}
                onClick={closeNavigationChrome}
              />
              <MenuAction
                label={t("nav.accountMenu.documentation")}
                icon={BookOpen}
                href={DOCS_URL}
                external
              ...
              {onShowShortcuts ? (
                <MenuAction
                  label={t("nav.accountMenu.keyboardShortcuts")}
                  icon={Keyboard}
                  onClick={() => {
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                aria-label={t("nav.accountMenu.shareFeedback")}
            ...
            <TooltipContent side="top">{t("nav.accountMenu.shareFeedback")}</TooltipContent>
          </Tooltip>
        ) : null}
      </div>
    </div>
