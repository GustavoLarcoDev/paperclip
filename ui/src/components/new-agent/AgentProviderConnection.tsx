...
export function AgentProviderConnection({
  ...
  async function connect() {
    ...
    try {
      ...
      if (connected) onConnected(connection);
        ...
        setError(
          t("newAgent.connection.providerNoResponse"),
        );
    } catch (cause) {
      if (run !== epoch.current) return;
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : t("newAgent.connection.couldNotConnect"),
      );
    } finally {
  ...
  return (
    <div className="min-w-0 max-w-full">
      <ModelSourceTiles
        label={t("newAgent.connection.connectProviderLabel")}
        sources={[
          {
            id: adapterType,
            label: provider,
      ...
      {!opened && savedKeys.options.length > 0 && (
        <p className="mt-2 text-sm text-muted-foreground">
          {t("newAgent.connection.savedApiKeys", { count: savedKeys.options.length })}
        </p>
      )}
      {method === "subscription" &&
            ...
            {method === "api" ? (
              <OnboardingLoginCard
                instruction={
                  savedKeys.options.length
                    ? t("newAgent.connection.chooseSavedKey")
                    : t("newAgent.connection.provideKey", { provider })
                }
              >
                <SavedProviderKeySelect
                  {...savedKeys}
                ...
                {!selectedKey && (
                  <OnboardingCardField
                    label={t("newAgent.connection.apiKey")}
                    masked
                    autoFocus
                    value={apiKey}
                    placeholder={
                      storedConnection
                        ? t("newAgent.connection.keyEnteredRetry")
                        : t("newAgent.connection.enterApiKey")
                    }
                    onChange={(value) => {
                      setSelectedKeyId("");
                      setApiKey(value);
              ...
              <AdapterLoginPanel
                ...
                onConnected={(sessionId) => {
                  if (managedAccount) {
                    if (!sessionId) { setError(t("newAgent.connection.loginNoConnection")); return; }
                    const run = epoch.current;
                    setLoginPhase("connecting");
                    void aiConnectionsApi.loginResult(companyId, sessionId).then((result) => {
                      ...
                      if (run !== epoch.current) return;
                      setLoginPhase("ready");
                      setError(t("newAgent.connection.couldNotRetrieveConnection"));
                    });
                    return;
                  }
              ...
              <p className="text-sm text-muted-foreground">
                {storedLogin.data
                  ? t("newAgent.connection.useSavedClaude")
                  : canLogin
                    ? t("newAgent.connection.useExistingConnection")
                    : t("newAgent.connection.noBrowserSignIn")}
              </p>
            )}
          </div>
        )}
      ...
      {method === "subscription" && storedLogin.isError && (
        <p role="alert" className="mt-4 text-sm text-destructive">
          {t("newAgent.connection.couldNotCheckClaude")}
        </p>
      )}
      {error && (
        <p role="alert" className="mt-4 text-sm text-destructive">
      ...
      {localEnvironment && health.isError && (
        <p role="alert" className="mt-4 text-sm text-destructive">{t("newAgent.connection.couldNotPrepareSignIn")}</p>
      )}
      <FooterNav
        onBack={() => {
          if (opened) cancel();
        ...
        primaryLabel={
          opened && needsLogin
            ? loginPhase === "waiting" ? t("newAgent.connection.waitingForCode")
              : loginPhase === "connecting" ? t("newAgent.connection.connecting")
              : t("newAgent.connection.signInTo", { provider })
            : busy
            ? t("newAgent.connection.connecting")
            : method === "subscription" &&
                (storedLogin.data || savedSubscription)
              ? t("newAgent.connection.useSavedSubscription")
              : method === "api" && selectedKey
                ? t("newAgent.connection.useSavedApiKey")
                : t("newAgent.connection.connect")
        }
        primaryDisabled={
