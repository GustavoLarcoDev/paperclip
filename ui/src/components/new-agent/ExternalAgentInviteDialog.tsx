...
export function ExternalAgentInviteDialog({ companyId, onClose, onBack }: {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
    <DialogContent className="max-h-(--sz-calc-16) overflow-y-auto sm:max-w-2xl">
      <DialogTitle>{prompt ? t("newAgent.invite.promptTitle") : t("newAgent.invite.inviteTitle")}</DialogTitle>
      <DialogDescription>
        {prompt ? t("newAgent.invite.promptDescription")
          : t("newAgent.invite.inviteDescription")}
      </DialogDescription>
      {prompt ? <>
        <Textarea aria-label={t("newAgent.invite.promptTitle")} readOnly value={prompt} className="min-h-64 font-mono text-xs" />
        {copyError && <p role="alert" className="text-sm text-muted-foreground">{t("newAgent.invite.clipboardUnavailable")}</p>}
        <div className="flex justify-between gap-4">
          <Button variant="ghost" onClick={onClose}>{t("newAgent.invite.done")}</Button>
          <Button variant="outline" onClick={() => void copy(prompt)}>{copied ? t("newAgent.invite.copiedPrompt") : t("newAgent.invite.copyPrompt")}</Button>
        </div>
      </> : <>
        <label className="space-y-2 text-sm">
          <span>{t("newAgent.invite.optionalMessage")}</span>
        ...
        <div className="flex justify-between gap-4">
          <Button variant="ghost" onClick={onBack}>{t("newAgent.invite.back")}</Button>
          <Button disabled={createInvite.isPending} onClick={() => createInvite.mutate()}>
            {createInvite.isPending ? t("newAgent.invite.generating") : t("newAgent.invite.generatePrompt")}
          </Button>
        </div>
      </>}
    </DialogContent>
