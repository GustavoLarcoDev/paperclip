      ...
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <p className="truncate text-sm font-semibold text-foreground">{t("dashboard.smokeLab.title")}</p>
          </div>
          <p className="mt-0.5 truncate text-xs text-muted-foreground">
            {t(`dashboard.smokeLab.health.${health}`)}
            {failing.length > 0 && `: ${failing.join(", ")}`}
          </p>
          <p className="mt-0.5 truncate text-(length:--text-micro) text-muted-foreground/80">
            {latestRun
              ? t("dashboard.smokeLab.lastRun", { time: formatTime(latestRun.startedAt) })
              : t("dashboard.smokeLab.runFromTab")}
          </p>
        </div>
      </div>
