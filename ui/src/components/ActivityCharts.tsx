...
function runDayTooltip(entry: DashboardRunActivityDay, t: TFunction): string {
  const lines = [t("dashboard.charts.runDayTooltip.total", { date: entry.date, count: entry.total })];
  if (entry.succeeded > 0) lines.push(`  ${t("dashboard.charts.runDayTooltip.succeeded", { count: entry.succeeded })}`);
  if (entry.recovered > 0) lines.push(`  ${t("dashboard.charts.runDayTooltip.recovered", { count: entry.recovered })}`);
  if (entry.failed > 0) {
    lines.push(`  ${t("dashboard.charts.runDayTooltip.failed", { count: entry.failed })}`);
    const codes = Object.entries(entry.failedByErrorCode ?? {}).sort((a, b) => b[1] - a[1]);
    for (const [code, count] of codes) lines.push(`    ${code}: ${count}`);
  }
  if (entry.other > 0) lines.push(`  ${t("dashboard.charts.runDayTooltip.other", { count: entry.other })}`);
  return lines.join("\n");
}
...
export function RunActivityChart(props: RunChartProps) {
  ...
  const legendItems = [
    { color: runSegmentColors.succeeded, label: t("dashboard.charts.runLegend.succeeded") },
    ...(hasRecovered ? [{ color: runSegmentColors.recovered, label: t("dashboard.charts.runLegend.recovered") }] : []),
    { color: runSegmentColors.failed, label: t("dashboard.charts.runLegend.failed") },
    { color: runSegmentColors.other, label: t("dashboard.charts.runLegend.other") },
  ];
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("dashboard.charts.noTasks")}</p>;
    ...
    <div>
      <div className="flex items-end gap-(--sz-3px) h-20">
        {days.map(day => {
          ...
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("dashboard.charts.dayTasksTooltip", { day, total })}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map(p => ({ color: priorityColors[p], label: t(`dashboard.charts.priority.${p}`) }))} />
    </div>
  );
}
...
// Statuses with a translated legend label (dashboard.charts.status.*);
// anything else shows its raw value.
const labeledStatuses = new Set(["todo", "in_progress", "in_review", "done", "blocked", "cancelled", "backlog"]);
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "var(--hex-6b7280)", label: labeledStatuses.has(s) ? t(`dashboard.charts.status.${s}`) : s }))} />
    </div>
  );
}
