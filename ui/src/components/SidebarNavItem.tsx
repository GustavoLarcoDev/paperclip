...
export function SidebarNavItem({
  ...
  const railStatusText = hasLive
    ? t("nav.navItem.live", { count: liveCount })
    : hasBadge
      ? badgeLabel ? t("nav.navItem.badgeWithLabel", { count: badge, label: badgeLabel }) : `${badge}`
      : alert
        ? t("nav.navItem.attentionNeeded")
        : undefined;
        ...
        <span className="ml-auto flex shrink-0 items-center gap-1.5 whitespace-nowrap">
          ...
          {hasLive && (
            <>
              ...
              <span className="text-(length:--text-micro) font-medium text-blue-600 dark:text-blue-400">{t("nav.navItem.live", { count: liveCount })}</span>
            </>
          )}
        </span>
      )}
