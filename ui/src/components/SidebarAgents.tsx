...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused ? t("nav.agents.resumeAgent") : t("nav.agents.pauseAgent");
  ...
  const trailingLabel = [
    showBuiltInLifecycle
      ? builtInStatus === "needs_setup" ? t("nav.agents.builtInNeedsSetup") : t("nav.agents.builtInPendingApproval")
      : null,
    hasInvalidOrgChain ? t("nav.agents.invalidReportingChain") : null,
  ...
  const navItem = (
    <SidebarNavItem
      ...
      liveAccessory={
        agent.pauseReason === "budget" ? <BudgetSidebarMarker title={t("nav.agents.pausedByBudget")} /> : undefined
      }
    />
  );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("nav.common.openActions", { name: agent.name })}
          >
        ...
        <DropdownMenuContent align="end" className="w-48">
          {onToggleStar ? (
            <>
              ...
              >
                ...
                <span>{starred ? t("nav.common.removeFromStarred") : t("nav.agents.starAgent")}</span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
            </>
          ) : null}
          <DropdownMenuItem asChild>
            ...
            >
              ...
              <span>{t("nav.agents.editAgent")}</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem
          ...
          >
            ...
            <span>{leaving ? t("nav.common.leaving") : t("nav.agents.leaveAgent")}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
...
export function SidebarAgents({ streamlined = false }: { streamlined?: boolean } = {}) {
  const { t } = useTranslation();
  const agentSortChoices: SidebarSectionRadioChoice[] = AGENT_SORT_VALUES.map((value) => ({
    value,
    label: t(`nav.sort.${value}`),
  }));
  ...
  const pauseResumeAgent = useMutation({
    ...
    onError: (error, { agent, action }) => {
      pushToast({
        title: action === "pause" ? t("nav.agents.pauseFailed") : t("nav.agents.resumeFailed"),
  ...
  return (
    <SidebarSection
      ...
      menu={{
        ...
        radioLabel: t("nav.agents.sortLabel"),
