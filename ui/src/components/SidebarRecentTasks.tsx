...
function RecentTasksList({
  ...
  const submitRename = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setRenameEntry(null);
      toastActions?.pushToast({ title: t("nav.recentTasks.renamed"), tone: "success" });
    } catch (error) {
      toastActions?.pushToast({
        title: t("nav.recentTasks.renameFailed"),
        body: errorMessage(error, t("nav.recentTasks.renameFailedBody")),
        tone: "error",
      });
    } finally {
  ...
  const archiveTask = async (entry: RecentTaskEntry) => {
    ...
    try {
      ...
      toastActions?.pushToast({ title: t("nav.recentTasks.archived"), tone: "success" });
    } catch (error) {
      toastActions?.pushToast({
        title: t("nav.recentTasks.archiveFailed"),
        body: errorMessage(error, t("nav.recentTasks.archiveFailedBody")),
        tone: "error",
      });
    } finally {
      setPendingAction(null);
    ...
    try {
      ...
      if (state.activePauseHold?.isRoot) {
        ...
        if (restartIssue.assigneeAgentId) {
          ...
          if (!("id" in wakeResult)) {
            throw new Error(wakeResult.message ?? t("nav.recentTasks.wakeSkipped"));
          }
        }
        setRestartWakeRetryPending(restartRetryStorageKey, entry.id, false);
        toastActions?.pushToast({ title: t("nav.recentTasks.restarted"), tone: "success" });
      } else if (state.activePauseHold) {
        throw new Error(t("nav.recentTasks.pausedByParent"));
        ...
        toastActions?.pushToast({ title: t("nav.recentTasks.paused"), tone: "success" });
      }
      await queryClient.invalidateQueries({
      ...
      toastActions?.pushToast({
        title: t("nav.recentTasks.pauseFailed"),
        body: errorMessage(error, t("nav.recentTasks.pauseFailedBody")),
        tone: "error",
      });
    } finally {
      setPendingAction(null);
  ...
  return (
    <>
      <SidebarSection label={t("nav.recentTasks.section")}>
        {entries.map((entry) => (
          <div key={entry.id} className="sidebar-action-row group/recent-task relative">
            <SidebarNavItem
              ...
              trailing={entry.status === "in_review" && entry.externalConversationState === "waiting"
                ? <span className="text-xs text-muted-foreground">{t("nav.recentTasks.idle")}</span> : undefined}
              trailingLabel={entry.status === "in_review" && entry.externalConversationState === "waiting" ? t("nav.recentTasks.idle") : undefined}
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    variant="ghost"
                    size="icon-xs"
                    aria-label={t("nav.recentTasks.moreActions", { title: entry.title })}
                ...
                >
                  ...
                  >
                    ...
                    {t("nav.recentTasks.pauseRestart")}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ) : null}
      ...
      >
        <DialogContent className="sm:max-w-md">
          <form className="grid gap-4" onSubmit={(event) => void submitRename(event)}>
            <DialogHeader>
              <DialogTitle>{t("nav.recentTasks.renameTitle")}</DialogTitle>
              <DialogDescription>{t("nav.recentTasks.renameDescription")}</DialogDescription>
            </DialogHeader>
            <Input
              autoFocus
              aria-label={t("nav.recentTasks.taskName")}
              value={renameValue}
            ...
            <DialogFooter>
              ...
              >
                {t("nav.recentTasks.cancel")}
              </Button>
              <Button
                type="submit"
                disabled={pendingAction === "rename" || !renameValue.trim()}
              >
                {pendingAction === "rename" ? t("nav.recentTasks.saving") : t("nav.recentTasks.save")}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
