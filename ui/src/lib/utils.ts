...
import { formatRelativeAgo } from "./timeAgo";
...
export function relativeTime(date: Date | string): string {
  ...
  if (diffSec < 60) return formatRelativeAgo(0, "second") ?? "just now";
  const diffMin = Math.round(diffSec / 60);
  if (diffMin < 60) return formatRelativeAgo(diffMin, "minute") ?? `${diffMin}m ago`;
  const diffHr = Math.round(diffMin / 60);
  if (diffHr < 24) return formatRelativeAgo(diffHr, "hour") ?? `${diffHr}h ago`;
  const diffDay = Math.round(diffHr / 24);
  if (diffDay < 30) return formatRelativeAgo(diffDay, "day") ?? `${diffDay}d ago`;
