...
export function Dashboard() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("dashboard.page.breadcrumb") }]);
  }, [setBreadcrumbs, t]);
    ...
    if (companies.length === 0) {
      return (
        <EmptyState
          ...
          message={t("dashboard.page.welcome")}
          action={t("dashboard.page.getStarted")}
          onAction={openOnboarding}
        />
      );
    }
    return (
      <EmptyState icon={LayoutDashboard} message={t("dashboard.page.selectOrganization")} />
    );
  }
    ...
    <div className="space-y-6">
      ...
      {pausedBanner?.kind === "imported" ? (
        <InlineBanner
          tone="warning"
          icon={PauseCircle}
          title={t("dashboard.pausedBanner.importedTitle", { count: pausedImportedCount })}
          actions={
            <Button
              size="sm"
            ...
            >
              {resumeImportedAgents.isPending ? t("dashboard.pausedBanner.resuming") : t("dashboard.pausedBanner.resumeAll")}
            </Button>
          }
        >
          {t("dashboard.pausedBanner.importedBody")}
        </InlineBanner>
      ) : pausedBanner?.kind === "all-paused" ? (
        <InlineBanner
          tone="warning"
          icon={PauseCircle}
          title={t("dashboard.pausedBanner.allPausedTitle")}
          actions={
            <Button variant="ghost" size="sm" asChild>
              <Link to="/agents">{t("dashboard.pausedBanner.reviewAgents")}</Link>
            </Button>
          }
        >
          {t("dashboard.pausedBanner.allPausedBody")}
        </InlineBanner>
      ) : null}
        ...
        <>
          {data.budgets.activeIncidents > 0 ? (
            <div className="flex items-start justify-between gap-3 rounded-xl border border-red-500/20 bg-(image:--gradient-extract-1) px-4 py-3">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-900/70 dark:text-red-100/70">
                    {t("dashboard.budgetIncidents.summary", {
              ...
              <Link to="/costs" className="text-sm underline underline-offset-2 text-red-900 dark:text-red-100">
                {t("dashboard.budgetIncidents.openBudgets")}
              </Link>
            </div>
          ) : null}
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-1 sm:gap-2">
            ...
            <MetricCard
              ...
              description={
                <span>
                  {t("dashboard.metrics.tasksDescription", { open: data.tasks.open, blocked: data.tasks.blocked })}
                </span>
              }
            />
            <MetricCard
              ...
              description={
                <span>
                  {data.costs.monthBudgetCents > 0
                    ? t("dashboard.metrics.budgetUtilization", {
              ...
              description={
                <span>
                  {data.budgets.pendingApprovals > 0
                    ? t("dashboard.metrics.budgetOverridesAwaiting", { overrides: data.budgets.pendingApprovals })
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {recentActivity.length > 0 && (
              <div className="min-w-0">
                <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">
                  {t("dashboard.page.recentActivity")}
                </h3>
            ...
            <div className="min-w-0">
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">
                {t("dashboard.page.recentTasks")}
              </h3>
              {recentIssues.length === 0 ? (
                <Card className="flex flex-wrap items-center justify-between gap-3 p-4">
                  <p className="text-sm text-muted-foreground">{t("dashboard.page.noTasks")}</p>
                  <Button size="sm" variant="outline" onClick={() => openNewIssue()}>
                    <SquarePen className="h-4 w-4" />
                    {t("dashboard.page.newTask")}
                  </Button>
                </Card>
              ) : (
                <Card className="@container block py-0 divide-y divide-border overflow-hidden">
